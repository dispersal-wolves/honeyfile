<p align="center">
  <img src="docs/banner.svg" alt="Honeyfile — Dispersal Wolves" width="100%">
</p>

# Honeyfile

**A small tripwire for files that should stay untouched.**

Creates and watches canary files for modification, movement, or deletion and can send generic webhook events.

## Start

```console
python src/honeyfile.py create ./important-record.txt
python src/honeyfile.py watch ./important-record.txt
```

For a global `honeyfile` command, run `python -m pip install .`.
...
This repository uses **Python** and the standard library only. Version 0.1 detects replacement, content changes, metadata changes, and deletion. Read-access detection is not claimed on platforms that cannot provide it reliably.
